...
function Steps({ steps }: { steps: string[][] }) {
  return <div className="steps-grid">{steps.map(([number, title, text]) => <article className="step-card" key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div>;
}
...
export function LandlordsPage() {
  ...
  return <AppShell><Seo title="Сдать квартиру спокойно и выгодно" description="Сервисный сценарий для собственников: прозрачная стоимость, подготовка объекта и квалификация заявок." /><section className="marketing-page">
    ...
    <section className="benefits-section"><p className="eyebrow">Почему Аренда БЕЗ</p><h2>Больше ясности на каждом этапе сдачи</h2><div className="benefits-grid"><article><CircleDollarSign /><h3>Без потери ставки</h3><p>Вы видите сумму, которую хотите получать, ещё до размещения.</p></article><article><Sparkles /><h3>Подготовка объявления</h3><p>Единая логика карточки и условий помогает сравнивать объекты честно.</p></article><article><HeartHandshake /><h3>Квалификация заявок</h3><p>Сначала базовая анкета, затем предложенные показы и понятный статус.</p></article></div></section>
    <section id="estimate" className="estimator-section"><div><p className="eyebrow">Оцените стоимость</p><h2>Посмотрите модель без скрытых вычетов</h2><p>Это локальный расчёт для сценария Figma. Реальные платежи и договоры не подключены в frontend-версии.</p><label htmlFor="owner-rate">Желаемая ставка, ₽ в месяц</label><input id="owner-rate" type="number" min="0" step="1000" value={rate} onChange={(event) => setRate(Number(event.target.value))} /></div><div className="economics-card"><div><span>Вы получаете</span><strong>{formatRubles(economics.ownerIncome)}</strong></div><div><span>Арендатор платит в месяц</span><strong>{formatRubles(economics.tenantMonthlyTotal)}</strong></div><div><span>Сервисный сбор</span><strong>{formatRubles(economics.serviceFee)}</strong></div></div></section>
    <section className="process-section"><div><p className="eyebrow">Как это работает</p><h2>Собираем сдачу в управляемый путь</h2></div><Steps steps={ownerSteps} /></section>
    <section className="owner-cta"><div><p className="eyebrow">Готовы начать?</p><h2>Расскажите о квартире — мы покажем следующий шаг</h2><p>В этой frontend-поставке вы увидите клиентский путь; проверка документов и модерация добавляются с серверной частью.</p></div><Link href="/auth?mode=register" className="button button--primary">Создать профиль <ArrowRight size={16} /></Link></section><Faq owner />
  </section></AppShell>;
}
