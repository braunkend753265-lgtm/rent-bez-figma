# Подтверждение Docker smoke test

> **Статус: успешно.** Docker-сборка и запуск контейнера выполнены на GitHub-hosted runner, так как Docker Engine недоступен в текущей среде разработки.

| Поле | Значение |
|---|---|
| Workflow | `Verify Docker image` |
| Run ID | `32319634992` |
| Статус | `completed` |
| Итог | `success` |
| Commit | `06e95a0a65f5b1766adcad8952ce784d944db5d3` |
| Запуск | [GitHub Actions run 32319634992](https://github.com/braunkend753265-lgtm/rent-bez-figma/actions/runs/32319634992) |
| Время завершения | 2026-08-20 01:04:35 UTC |

Workflow собрал образ командой `docker build -f for_you_Egor -t rent-bez:verify .`, запустил контейнер с портом `3000` и подтвердил успешный HTTP-ответ на `http://127.0.0.1:3000/`.

Повторная проверка запускается автоматически при изменениях Docker-конфигурации или исходного кода в `main`, а также вручную через GitHub Actions.

## Исправление контекста сборки

После того как локальная папка `node_modules` стала доступной Git, Docker workflow превысил лимит в 15 минут при передаче лишнего build context. Для исключения локальных зависимостей, кэшей, логов и VCS-данных добавлен `.dockerignore`. Зависимости устанавливаются внутри образа командой `pnpm install`, поэтому на работоспособность контейнера это не влияет, а объём входного контекста существенно сокращается.

| Повторная проверка | Значение |
|---|---|
| Run ID | `32401911419` |
| Итог | `success` |
| Длительность | 44 секунды |
| Запуск | [GitHub Actions run 32401911419](https://github.com/braunkend753265-lgtm/rent-bez-figma/actions/runs/32401911419) |

Повторный workflow подтвердил, что `.dockerignore` устранил проблему: Docker-образ успешно собран, контейнер запущен и HTTP smoke test пройден в пределах установленного лимита.
